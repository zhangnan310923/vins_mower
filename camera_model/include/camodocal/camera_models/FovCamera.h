#ifndef VINS_ESTIMATOR_CAMERA_MODELS_FOVCAMERA_H_
#define VINS_ESTIMATOR_CAMERA_MODELS_FOVCAMERA_H_
 ...
 public:
  class Parameters : public Camera::Parameters {
   public:
    Parameters();
    Parameters(
        const std::string& cameraName, int width, int height, double fx,
    ...
    //bool SetParams(const ninebot_algo::CameraParams& camera_params);
   ...
   * \brief Constructor from the projection model parameters
   */
  explicit FovCamera(const Parameters& params);
...
inline void OBSOLETE_IMPL_EXIT() {
  std::cout << "Attempt to use an obsolete implementation" << std::endl;
  assert(false);
  exit(-1);
}
...
#endif  // VINS_ESTIMATOR_CAMERA_MODELS_FOVCAMERA_H_

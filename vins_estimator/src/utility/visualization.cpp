...
ros::Publisher pub_odometry, pub_latest_odometry, pub_latest_wheel_odometry;
ros::Publisher pub_path, pub_relo_path, pub_gb_odom_path, pub_cache_odom_path, pub_cache_ENU_path;
ros::Publisher pub_point_cloud, pub_margin_cloud;
ros::Publisher pub_key_poses;
ros::Publisher pub_relo_relative_pose;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose_visual;
nav_msgs::Path path, relo_path, gb_path, cache_odom_path;

ros::Publisher pub_keyframe_pose;
ros::Publisher pub_keyframe_point;
ros::Publisher pub_extrinsic;

ros::Publisher pub_gb_odom_from_vrtk, pub_cache_odom, pub_cache_ENU;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_gb_odom_from_vrtk = n.advertise<nav_msgs::Odometry>("gb_odom_from_vrtk", 1000);
    pub_cache_odom = n.advertise<nav_msgs::Odometry>("cache_odom", 1000);
    pub_cache_ENU = n.advertise<nav_msgs::Odometry>("cache_enu", 1000);
    pub_gb_odom_path = n.advertise<nav_msgs::Path>("gb_path", 1000);
    pub_cache_odom_path = n.advertise<nav_msgs::Path>("cache_odom_path", 1000);
    pub_cache_ENU_path = n.advertise<nav_msgs::Path>("cache_enu_path", 1000);
...
void pubGbOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const std_msgs::Header &header)
{
    Eigen::Quaterniond quadrotor_Q = Q ;
    ...
    pub_gb_odom_from_vrtk.publish(odometry);
    ...
    gb_path.header = header;
    gb_path.header.frame_id = "world";
    gb_path.poses.push_back(pose_stamped);
    pub_gb_odom_path.publish(gb_path);
}

void pubCacheENU (const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const std_msgs::Header &header)
{
    Eigen::Quaterniond quadrotor_Q = Q ;
    ...
    odometry.twist.twist.linear.y = 0.0;
    odometry.twist.twist.linear.z = 0.0;
    pub_cache_ENU.publish(odometry);
    ...
    pose_stamped.header.frame_id = "world";
    pose_stamped.pose = odometry.pose.pose;
    cache_odom_path.header = header;
    cache_odom_path.header.frame_id = "world";
    cache_odom_path.poses.push_back(pose_stamped);
    pub_cache_ENU_path.publish(cache_odom_path);

    // write result to file
    ofstream foutC(GRONUD_TRUTH_PATH, ios::app);
...
void pubCacheOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const std_msgs::Header &header)
{
    Eigen::Quaterniond quadrotor_Q = Q ;
    ...
    odometry.twist.twist.linear.y = 0.0;
    odometry.twist.twist.linear.z = 0.0;
    pub_cache_odom.publish(odometry);
    ...
    cache_odom_path.header.frame_id = "world";
    cache_odom_path.poses.push_back(pose_stamped);
    pub_cache_odom_path.publish(cache_odom_path);
}

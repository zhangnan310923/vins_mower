...
void RefineGravityWithEncoder(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    Vector3d g0 = g.normalized() * G.norm();
    Vector3d lx, ly;
    //VectorXd x;
...
bool LinearAlignmentWithEncoder(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    int all_frame_count = all_image_frame.size();
    // int n_state = all_frame_count * 3 + 3 + 1;
    ...
    RefineGravityWithEncoder(all_image_frame, g, tmp_x);
...
bool VisualIMUAlignment(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs, Vector3d &g, VectorXd &x)
{
    ...
    if(ENCODER)
    {
        return LinearAlignmentWithEncoder(all_image_frame, g, x);
    }

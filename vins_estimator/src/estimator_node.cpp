...
#include "/home/zhangnn/vins_ws/devel/include/custom_msgs/Encoder.h"
...
deque<custom_msgs::EncoderConstPtr> en_buf;    //todo(znn) encoder_buf
...
void encoder_callback(const custom_msgs::EncoderConstPtr &encoder_msg)
{
    ...
    //estimator.processWheel(t, linear_velocity, angular_velocity);
    return;
}
...
{
    if(estimator.imu_count < 50)
    {
        estimator.Acc_Zero_Check.push_back(linear_acceleration);
        estimator.Gyr_Zero_Check.push_back(angular_velocity);
        estimator.acc_sum_ += linear_acceleration;
        estimator.gyr_sum_ += angular_velocity;
        estimator.imu_count++;
    }else
    {
        estimator.Acc_Zero_Check.push_back(linear_acceleration);
        estimator.Gyr_Zero_Check.push_back(angular_velocity);
        estimator.acc_sum_ = estimator.acc_sum_ + estimator.Acc_Zero_Check.back() - estimator.Acc_Zero_Check.front();
        estimator.gyr_sum_ = estimator.gyr_sum_ + estimator.Gyr_Zero_Check.back() - estimator.Gyr_Zero_Check.front();
        estimator.Acc_Zero_Check.pop_front();
        estimator.Gyr_Zero_Check.pop_front();
        Eigen::Vector3d avr_acc = estimator.acc_sum_ / estimator.imu_count;
        Eigen::Vector3d avr_gyr = estimator.gyr_sum_ / estimator.imu_count;
        double accnorm = avr_acc.squaredNorm();
        double gyrnorm = avr_gyr.squaredNorm();

        Eigen::Vector3d var_acc;
        Eigen::Vector3d var_gyr;
        for(auto &acc_iter : estimator.Acc_Zero_Check)
        {
        ...
        for(auto &gyr_iter : estimator.Gyr_Zero_Check)
        {
        ...
        var_acc /= estimator.imu_count;
        var_gyr /= estimator.imu_count;
...
/*         ROS_WARN_STREAM("gyrnorm = " << gyrnorm);
        ...
        if((gyrnorm <= 0.005) && (var_acc.x() <= 0.01) && (var_acc.y() <= 0.1))
...
std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, 
            std::vector<custom_msgs::EncoderConstPtr>>> getMeasurements()
{
    std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, 
    ...
    while(true)
    {
        ...
        for(auto it = encoder_buf.begin(); it != encoder_buf.end(); it++)
        {
            if((*it)->header.stamp.toSec() < IMUs.back()->header.stamp.toSec())
            {
                encoders.emplace_back(*it);
            }else
            {
                encoders.emplace_back(*it);
                break;
...
void process()
{
    while(true)
    {
        ...
        for(auto &measurement : measurements)
        {
            ...
            {
                auto begin_encoder_msg = encoder_measurement[i - 1];
                auto end_encoder_msg = encoder_measurement[i];
                double dt = end_encoder_msg->header.stamp.toSec() - begin_encoder_msg->header.stamp.toSec();
                double encoder_right_vel = (double)(end_encoder_msg->right_encoder - begin_encoder_msg->right_encoder) /  ENC_RESOLUTION * M_PI * RIGHT_D / dt;
                double encoder_left_vel = (double)(end_encoder_msg->left_encoder - begin_encoder_msg->left_encoder) / ENC_RESOLUTION * M_PI * LEFT_D / dt;
                double encoder_vel = 0.5 * (encoder_right_vel + encoder_left_vel);
                double encoder_omega = (encoder_right_vel - encoder_left_vel) / WHEELBASE;

                Eigen::Vector3d tmp_enconder_vel(encoder_vel, 0, 0);
                Eigen::AngleAxisd tmp_rot_vec(encoder_omega * dt, Eigen::Vector3d::UnitY());
                //no rotation -- not accurate enough
                Eigen::Vector3d encoder_velocity(tmp_enconder_vel);
                double timestamp = 0.5 * (end_encoder_msg->header.stamp.toSec() + begin_encoder_msg->header.stamp.toSec());
                encoder_velocities.emplace_back(timestamp, encoder_velocity);
            }
            ...
            for(auto &imu_msg : std::get<0>(measurement))
            {
                ...
                std::pair<double, Eigen::Vector3d> encoder_vel_0, encoder_vel_1;
                for(auto &encoder_vel : encoder_velocities)
                {
                    if(encoder_vel.first <= t)
                    {
                        t1 = encoder_vel.first;
                        encoder_vel_0 = encoder_vel;
                    }else
                    {
                        t2 = encoder_vel.first;
                        encoder_vel_1 = encoder_vel;
                        break;
                    }  
                }
                if(t1 > 0 && t2 > 0)
                {
                    ...
                    ROS_ASSERT(t_f >= 0);
                    ROS_ASSERT(t_b >= 0);
                    double w1 = t_b / (t_f + t_b);
                    double w2 = t_f / (t_f + t_b);
                    encoder_velocity = w1 * encoder_vel_0.second + w2 * encoder_vel_1.second;
                }
                ...
                if(t < img_t)
                {
                    ...
                    estimator.processIMUEncoder(dt, Eigen::Vector3d(dx, dy, dz), Eigen::Vector3d(rx, ry, rz), 
                                                    Eigen::Vector3d(vx, vy, vz));
                }else
                {
                    double t_f = img_t - current_time;
                    double t_b = t - img_t;
                    current_time = img_t;
                    ROS_ASSERT(t_f >= 0);
                    ROS_ASSERT(t_b >= 0);
                    ...
                    estimator.processIMUEncoder(t_f, Eigen::Vector3d(dx, dy, dz), Eigen::Vector3d(rx, ry, rz), 
                                                     Eigen::Vector3d(vx, vy, vz));                   
                }  
            }
            encoder_velocities.clear();
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                { 
                    ...
                    ZUPT(Vector3d(dx, dy, dz), Vector3d(rx, ry, rz));
...
int main(int argc, char **argv)
{
    ...
    readParameters(n);
    estimator.setParameter();
    Eigen::Vector3d eulerAngle(-1.57079632679, 0, 0);       //euler2rotation RPY
    ...
    ROS_WARN_STREAM("rotation_matrix = " << rotation_matrix);

...
//        delta_v.setZero();
    ...
    //Ô¤»ý·Ö£¬ÇÒ½øÐÐÁËÔ¤»ý·ÖµÄÐ­·½²îÀÛ¼Æ£¬½øÐÐÁËÔ¤»ý·ÖÏà¶ÔbiasµÄ±ä»¯µÄ±ä»¯ÑÅ¿É±ÈµÄ¼ÆËã
...
//            jacobian.setZero();
            covariance = F * covariance * F.transpose() + V * noise * V.transpose();//Ð­·½²î´«µÝ
        }
...
//        delta_v = result_delta_v;
    ...
    //evaulateµÄ²ÎÊýÓ¦¸Ã°üº¬ËùÓÐÓë¸ÃÒò×ÓÏàÁ¬µÄ½Úµã
...
//        residuals.block<3, 1>(O_BG, 0) = Bgj - Bgi;
    ...
    const Eigen::Vector3d linearized_vel, linearized_gyr;//Ô¤»ý·ÖÆðÊ¼ÂÖËÙÊý¾Ý
    double linearized_sx, linearized_sy, linearized_sw;
    double linearized_td;
    Eigen::Matrix<double, 6, 3> jacobian;
    Eigen::Matrix<double, 6, 3> step_jacobian_ix;//ÂÖËÙÄÚ²ÎµÄstep_jacobian
